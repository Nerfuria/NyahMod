...
public class WynncraftAPI {
    ...
    public static Map<String, TerritoryResponse> territoryResponse() {
        ...
        return gson.fromJson(WebUtils.queryAPI(NyahConfig.getData().getApiBase() + "guild/list/territory"), type);
    }

    public static GuildResponse guildResponse(String guildName) {
        return gson.fromJson(WebUtils.queryAPI(NyahConfig.getData().getApiBase() + "guild/%s?identifier=username".formatted(guildName.replace(" ", "%20"))), GuildResponse.class);
    }
}
